...
/* ══════════════════════════════════════════════════════════
   ESCENARIO B — EULER / HEUN / RK4  (DESCENSO)
   ══════════════════════════════════════════════════════════ */

/** Ejecuta UN método EDO escalar y devuelve {ts, ys, tCruce}. */
function _correrEDO(metodo, f, H0, alarma, h) {
  const solvers = { euler: NUM.euler, heun: NUM.heun, rk4: NUM.rk4 };
  const { ts, ys } = solvers[metodo](f, H0, 0, 3600, h, (t, H) => H <= alarma);
  let tCruce = null;
...
/** Submuestrea arrays largos a máx nMax puntos. */
function _sub(arr, nMax = 300) {
  const step = Math.max(1, Math.floor(arr.length / nMax));
  return arr.filter((_, i) => i % step === 0);
}

function simularDescenso(H0val, gravVal, empVal, alarmaVal, hVal, resId, chartId, metodoForced) {
  resId   = resId   || 'resB';
  chartId = chartId || 'chartB';
  try {
    ...
    const metodo = metodoForced || (document.getElementById('metodoB') ? document.getElementById('metodoB').value : 'rk4');

    if (h <= 0)    throw new Error('El paso h debe ser positivo');
    if (H0 <= alarma) throw new Error('La altitud inicial debe superar la alarma');

    // dH/dt = Empuje − Gravedad  (constante aquí, variable si se extiende)
    const f = (t, H) => empuje - grav;

    const COLORS = { euler: '#ff6b35', heun: '#00e896', rk4: '#00d4ff' };
    const NOMBRES = { euler: 'Euler', heun: 'Heun (RK2)', rk4: 'RK4' };

    if (metodo === 'todos') {
      // ── MODO COMPARACIÓN ──────────────────────────────
      const resultados = {};
      ['euler', 'heun', 'rk4'].forEach(m => { resultados[m] = _correrEDO(m, f, H0, alarma, h); });

      // Usar eje de tiempo del más largo (rk4 como referencia)
      const tsRef = _sub(resultados.rk4.ts);

      let texto = `<span class="hi">Comparación de métodos EDO</span>\n\n`;
      ['euler', 'heun', 'rk4'].forEach(m => {
        const tc = resultados[m].tCruce;
        texto += `${NOMBRES[m].padEnd(12)}: t_cruce = ${tc !== null ? tc.toFixed(4) + ' s' : 'No alcanzó'}\n`;
      });
      const tc_rk4 = resultados.rk4.tCruce, tc_eu = resultados.euler.tCruce, tc_h = resultados.heun.tCruce;
      if (tc_rk4 && tc_eu) texto += `\nDesviación Euler vs RK4:  ${Math.abs(tc_eu - tc_rk4).toFixed(4)} s`;
      if (tc_rk4 && tc_h)  texto += `\nDesviación Heun  vs RK4:  ${Math.abs(tc_h - tc_rk4).toFixed(4)} s`;
      texto += `\n\n<span class="ok">RK4 es la referencia más precisa (error O(h⁵)/paso).</span>`;
      setResult(resId, texto);

      // Tabla comparativa
      const wrap = document.getElementById('tablaB-wrap');
      if (wrap) {
        wrap.style.display = '';
        document.getElementById('tablaB').innerHTML = `
          <table><thead><tr>
            <th>Método</th><th>Orden</th><th>Evaluaciones f/paso</th><th>t cruce (s)</th><th>Error vs RK4 (s)</th>
          </tr></thead><tbody>
          ${[['euler','1','1'],['heun','2','2'],['rk4','4','4']].map(([m, ord, ev]) => {
            const tc = resultados[m].tCruce;
            const err = tc && tc_rk4 ? Math.abs(tc - tc_rk4).toFixed(6) : '—';
            return `<tr${m==='rk4'?' class="converged"':''}>
              <td>${NOMBRES[m]}</td><td>${ord}</td><td>${ev}</td>
              <td>${tc !== null ? tc.toFixed(4) : 'No alcanzó'}</td>
              <td>${err}</td></tr>`;
          }).join('')}
          </tbody></table>`;
      }

      // Gráfica con las 3 curvas + línea de alarma
      const datasets = ['euler', 'heun', 'rk4'].map(m => ({
        label: NOMBRES[m],
        data: _sub(resultados[m].ys),
        borderColor: COLORS[m],
        pointRadius: 0,
        borderWidth: m === 'rk4' ? 2.5 : 1.5,
        borderDash: m === 'euler' ? [4, 3] : m === 'heun' ? [2, 2] : [],
        tension: 0.3,
      }));
      datasets.push({
        label: `Alarma (${alarma} m)`,
        data: tsRef.map(() => alarma),
        borderColor: '#ff4444',
        borderDash: [6, 4],
        pointRadius: 0,
        borderWidth: 1.5,
      ...
      crearChart(chartId, {
        type: 'line',
        data: { labels: tsRef, datasets },
        options: { ...defaultOpts('Comparación Euler · Heun · RK4'), scales: {
          x: { ...defaultOpts().scales.x, title: { display: true, text: 'Tiempo (s)', color: tickColor } },
          y: { ...defaultOpts().scales.y, title: { display: true, text: 'Altitud (m)', color: tickColor } }
        }}
      });

    } else {
      // ── MODO ÚNICO ────────────────────────────────────
      const { ts, ys, tCruce } = _correrEDO(metodo, f, H0, alarma, h);
      const tsPlot = _sub(ts), ysPlot = _sub(ys);

      const resText = tCruce !== null
        ? `<span class="ok">✓ ${NOMBRES[metodo]} — Simulación completada</span>\n\n` +
          `<span class="hi">Tiempo de cruce a zona crítica:</span>\n` +
          `  t = ${tCruce.toFixed(4)} s (${(tCruce/60).toFixed(4)} min)\n\n` +
          `Altitud final: ${ys[ys.length-1].toFixed(1)} m\n` +
          `Velocidad neta de descenso: ${(grav - empuje).toFixed(1)} m/s\n` +
          `Pasos de integración: ${ts.length}\n\n` +
          `Interpretación: El FANI tardó ${tCruce.toFixed(2)} s en penetrar\nel espacio aéreo comercial (${alarma} m).`
      ...
      crearChart(chartId, {
        ...
        data: {
          labels: tsPlot,
          datasets: [
            { label: `Altitud — ${NOMBRES[metodo]}`, data: ysPlot,
              borderColor: COLORS[metodo], backgroundColor: `${COLORS[metodo]}14`,
              fill: true, tension: 0.4, pointRadius: 0, borderWidth: 2 },
            { label: `Zona de alerta (${alarma} m)`, data: tsPlot.map(() => alarma),
              borderColor: '#ff6b35', borderDash: [6, 4], pointRadius: 0, borderWidth: 1.5 }
          ]
        },
        options: { ...defaultOpts(`Descenso Cinemático — ${NOMBRES[metodo]}`), scales: {
          x: { ...defaultOpts().scales.x, title: { display: true, text: 'Tiempo (s)', color: tickColor } },
          y: { ...defaultOpts().scales.y, title: { display: true, text: 'Altitud (m)', color: tickColor } }
        }}
      });
...
/* ══════════════════════════════════════════════════════════
   ESCENARIO C — INTERPOLACIÓN (Lagrange · Newton · Spline)
   ══════════════════════════════════════════════════════════ */
function interpolar(xsIn, ysIn, tConsIn, resId, chartId) {
  resId = resId || 'resC'; chartId = chartId || 'chartC';
  try {
    let xs, ys, tConsulta, metodo;
    if (xsIn !== undefined) {
      xs = xsIn; ys = ysIn; tConsulta = tConsIn;
      metodo = 'spline'; // casos de prueba usan spline por defecto
    } else {
      const pts = parsePuntos('puntosC');
      xs = pts.xs; ys = pts.ys;
      tConsulta = getNum('tConsulta', 'Segundo consulta');
      metodo = document.getElementById('metodoC') ? document.getElementById('metodoC').value : 'spline';
    }
    ...
    const evalLagrange = t => NUM.lagrange(sxs, sys, t);
    const evalNewton   = t => NUM.newtonDivDiff(sxs, sys, t).value;

    const METODOS = {
      spline:   { fn: evalSpline,   label: 'Spline Cúbico',          color: '#7b2fff' },
      lagrange: { fn: evalLagrange, label: 'Lagrange',               color: '#00e896' },
      newton:   { fn: evalNewton,   label: 'Newton (Dif. Divididas)', color: '#ff6b35' },
    };

    // Puntos sensores para scatter
    const sensorDataset = {
      label: 'Datos sensor',
      data: tCurva.map(t => {
    ...
    if (metodo === 'todos') {
      // ── COMPARACIÓN ───────────────────────────────────
      const valores = {};
      Object.entries(METODOS).forEach(([k, { fn }]) => { valores[k] = fn(tConsulta); });

      let texto = `<span class="hi">Comparación de métodos — t = ${tConsulta} s</span>\n\n`;
      Object.entries(METODOS).forEach(([k, { label }]) => {
        texto += `${label.padEnd(28)}: ${valores[k].toFixed(4)} m\n`;
      });
      const ref = valores.spline;
      texto += `\nDesviación Lagrange vs Spline: ${Math.abs(valores.lagrange - ref).toFixed(4)} m`;
      texto += `\nDesviación Newton   vs Spline: ${Math.abs(valores.newton   - ref).toFixed(4)} m`;
      texto += `\n\n<span class="ok">Con pocos puntos bien distribuidos los tres métodos convergen al mismo valor.</span>`;
      setResult(resId, texto);

      // Tabla de diferencias divididas de Newton
      const { tabla } = NUM.newtonDivDiff(sxs, sys, tConsulta);
      const tablaWrap = document.getElementById('tablaC-wrap');
      if (tablaWrap) {
        tablaWrap.style.display = '';
        let tHtml = '<table><thead><tr><th>i</th><th>xᵢ</th><th>f[xᵢ]</th>';
        for (let j = 1; j < sxs.length; j++) tHtml += `<th>Δ${j}</th>`;
        tHtml += '</tr></thead><tbody>';
        for (let i = 0; i < sxs.length; i++) {
          tHtml += `<tr><td>${i}</td><td>${sxs[i].toFixed(2)}</td>`;
          for (let j = 0; j < sxs.length; j++) {
            tHtml += `<td>${tabla[i][j] !== 0 || j === 0 ? tabla[i][j].toFixed(4) : ''}</td>`;
          }
          tHtml += '</tr>';
        }
        tHtml += '</tbody></table>';
        document.getElementById('tablaC').innerHTML = tHtml;
      }

      // 3 curvas + scatter + punto consultado (promedio)
      const datasets = Object.entries(METODOS).map(([k, { fn, label, color }]) => ({
        label,
        data: tCurva.map(t => fn(t)),
        borderColor: color,
        pointRadius: 0,
        borderWidth: k === 'spline' ? 2.5 : 1.8,
        borderDash: k === 'lagrange' ? [5,3] : k === 'newton' ? [2,2] : [],
        tension: 0,
      }));
      ...
      crearChart(chartId, {
        type: 'line',
        data: { labels: tCurva.map(t => t.toFixed(2)), datasets },
        options: { ...defaultOpts(`Comparación de Métodos — t=${tConsulta}s`), scales: {
          x: { ...defaultOpts().scales.x, title: { display: true, text: 'Tiempo (s)', color: tickColor } },
          y: { ...defaultOpts().scales.y, title: { display: true, text: 'Altitud (m)', color: tickColor } }
        }}
      });
      ...
      // ── MODO ÚNICO ────────────────────────────────────
      if (metodo === 'spline' && xs.length < 3) throw new Error('Spline necesita mínimo 3 puntos');
      const { fn, label, color } = METODOS[metodo];
      const yInterp = fn(tConsulta);
      const yCurva  = tCurva.map(t => fn(t));

      // Tabla de diferencias divididas solo para Newton
      const tablaWrap = document.getElementById('tablaC-wrap');
      if (metodo === 'newton' && tablaWrap) {
        tablaWrap.style.display = '';
        const { tabla } = NUM.newtonDivDiff(sxs, sys, tConsulta);
        let tHtml = '<table><thead><tr><th>i</th><th>xᵢ</th><th>f[xᵢ]</th>';
        for (let j = 1; j < sxs.length; j++) tHtml += `<th>Δ${j}</th>`;
      ...
      setResult(resId,
        ...
        `Interpretación: El objeto se encontraba a ${yInterp.toFixed(1)} m\nen el segundo ${tConsulta}.`
      );
      ...
      crearChart(chartId, {
        ...
        data: {
          ...
          datasets: [
            { label, data: yCurva, borderColor: color, backgroundColor: `${color}14`,
            ...
            { label: `t=${tConsulta}s → ${yInterp.toFixed(1)}m`,
              ...
              borderColor: '#ff6b35', backgroundColor: '#ff6b35', pointRadius: 9, showLine: false }
          ]
        },
        options: { ...defaultOpts(`Ruta de Vuelo — ${label}`), scales: {
          x: { ...defaultOpts().scales.x, title: { display: true, text: 'Tiempo (s)', color: tickColor } },
          y: { ...defaultOpts().scales.y, title: { display: true, text: 'Altitud (m)', color: tickColor } }
        }}
      });
...
/* ══════════════════════════════════════════════════════════
   ESCENARIO G — DINÁMICA SOCIAL  (Euler · Heun · RK4 vectorizados)
   ══════════════════════════════════════════════════════════ */
function simularDinamica() {
  try {
    ...
    const metodo = document.getElementById('metodoG').value;
    ...
    const SOLVERS = {
      euler: NUM.eulerVector,
      heun:  NUM.heunVector,
      rk4:   NUM.rk4Vector,
    };
    const NOMBRES_G = { euler: 'Euler', heun: 'Heun (RK2)', rk4: 'RK4' };
    const COLORS_G  = { euler: '#ff6b35', heun: '#00e896', rk4: '#00d4ff' };

    // Submuestrear helper
    const subY = (Ys, nMax = 300) => {
      const step = Math.max(1, Math.floor(Ys.length / nMax));
      return Ys.filter((_, i) => i % step === 0);
    };

    if (metodo === 'todos') {
      // ── COMPARACIÓN: mostrar curva de Alarmados (A) para los 3 métodos ──
      const resultados = {};
      Object.entries(SOLVERS).forEach(([m, solver]) => {
        const { ts, Ys } = solver(F, Y0, 0, T, h);
        resultados[m] = { ts: _sub(ts), Ys: subY(Ys) };
      });

      // Estado final de cada uno
      let texto = `<span class="hi">Comparación de métodos — Alarmados(A) al día ${T}</span>\n\n`;
      Object.entries(NOMBRES_G).forEach(([m, nm]) => {
        const Yfin = resultados[m].Ys[resultados[m].Ys.length - 1];
        const total = Yfin[0] + Yfin[1] + Yfin[2];
        texto += `${nm.padEnd(14)}: A_fin = ${Yfin[1].toFixed(1)} (${(Yfin[1]/total*100).toFixed(1)}%)\n`;
      });
      const A_rk4   = resultados.rk4.Ys[resultados.rk4.Ys.length-1][1];
      const A_euler = resultados.euler.Ys[resultados.euler.Ys.length-1][1];
      const A_heun  = resultados.heun.Ys[resultados.heun.Ys.length-1][1];
      texto += `\nDesviación Euler vs RK4: ${Math.abs(A_euler - A_rk4).toFixed(2)} personas`;
      texto += `\nDesviación Heun  vs RK4: ${Math.abs(A_heun  - A_rk4).toFixed(2)} personas`;
      texto += `\n\n<span class="ok">RK4 vectorizado es el más preciso para sistemas no lineales.</span>`;
      setResult('resG', texto);

      const datasets = Object.entries(NOMBRES_G).map(([m, nm]) => ({
        label: `Alarmados — ${nm}`,
        data: resultados[m].Ys.map(Y => Y[1]),
        borderColor: COLORS_G[m],
        pointRadius: 0,
        borderWidth: m === 'rk4' ? 2.5 : 1.8,
        borderDash: m === 'euler' ? [5,3] : m === 'heun' ? [2,2] : [],
        tension: 0.3,
      }));

      crearChart('chartG', {
        type: 'line',
        data: { labels: resultados.rk4.ts.map(t => (+t).toFixed(1)), datasets },
        options: { ...defaultOpts('Comparación: Alarmados (A) — Euler · Heun · RK4'), scales: {
          x: { ...defaultOpts().scales.x, title: { display: true, text: 'Días', color: tickColor } },
          y: { ...defaultOpts().scales.y, title: { display: true, text: 'Personas', color: tickColor } }
        }}
      });

    } else {
      // ── MODO ÚNICO ────────────────────────────────────
      const { ts, Ys } = SOLVERS[metodo](F, Y0, 0, T, h);
      const tsP = _sub(ts), YsP = subY(Ys);
      ...
      let tendencia;
      if (Afin / total > 0.5)    tendencia = 'MASIFICACIÓN — El pánico social domina la población.';
      ...
      setResult('resG',
        `<span class="ok">✓ ${NOMBRES_G[metodo]} vectorizado — ${ts.length} pasos</span>\n\n` +
        `Estado final (día ${T}):\n` +
      ...
      crearChart('chartG', {
        ...
        options: { ...defaultOpts(`Dinámica Social — ${NOMBRES_G[metodo]}`), scales: {
          x: { ...defaultOpts().scales.x, title: { display: true, text: 'Días', color: tickColor } },
          y: { ...defaultOpts().scales.y, title: { display: true, text: 'Población', color: tickColor } }
        }}
      });
    }
  } catch (e) {
